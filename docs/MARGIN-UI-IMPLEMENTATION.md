...
Actions:
selectProject(id) [parent changes active selection locally, no chat submission], newProject [a turn offering /new or /register], refreshProjects [retry the first project read], plan(id), play(id), fix(id) [composer /fix], review(id), autoMode(id,value off|suggest|stage|ship), spendCap(id,number), providers(id) [advanced settings Providers], voice, sounds, notifications, model [Providers], settings [open compact quick-settings card inside module; NOT callback], advancedSettings [advanced settings modal], calm, demo, tidy.
