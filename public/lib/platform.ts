  ...
  const show = async (tab: string): Promise<void> => {
    ...
    try {
      if (tab === 'Providers') {
        ...
        // First in the tab, not under the project commands: a lapsed sign-in sends you here, and the way
        // to sign in was below the fold at 1180x820 and three screens down on a phone.
        content.prepend(auth, connection, node('p', 'Claude uses your Claude Code sign-in on this Mac. No API key is required. Sign-in opens the official Claude Code flow in Terminal and your browser; your credentials stay with Claude Code. Plan limits apply.'));
        // The check runs when the tab opens: the question the tab exists to answer is "am I signed in?",
        // and it used to wait behind a button. It is not run at boot — it starts the claude CLI and a Codex RPC.
        // The line says what happened last. The check takes seconds against real accounts, and one
        // started as the tab opened used to land after "Sign in with Claude" and write over its instructions.
        let latest = 0;
        const say = async (ask: () => Promise<string>): Promise<void> => { const mine = ++latest; const text = await ask(); if (mine === latest) connection.textContent = text; };
        const check = (): Promise<void> => say(async () => { const status = await api.get('/api/providers'); return 'Claude: ' + (status.claude.connected ? (status.claude.mode === 'api-key' ? 'explicit API-key mode' : 'signed in' + (status.claude.subscription ? ' · ' + status.claude.subscription : '')) : status.claude.error || 'sign in required') + ' · Codex: ' + (status.codex.connected ? 'connected' : status.codex.error || 'sign in required'); });
        connection.textContent = 'Checking your sign-ins…'; const opening = latest + 1; void check().catch(error => { if (latest === opening) connection.textContent = (error as Error).message; });
        auth.append(action('Check again', check), action('Sign in with Claude', () => say(async () => (await api.post('/api/providers/claude/login')).message)), action('Connect Codex', async () => { const result = await api.post('/api/providers/login'); const link = node('a', 'Continue Codex sign-in'); link.href = result.authUrl; link.target = '_blank'; link.rel = 'noopener noreferrer'; auth.append(link); }));
