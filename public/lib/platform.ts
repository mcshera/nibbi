  ...
  const show = async (tab: string): Promise<void> => {
    ...
    try {
      if (tab === 'Providers') {
        ...
        const check = async (): Promise<void> => { const status = await api.get('/api/providers'); connection.textContent = 'Claude: ' + (status.claude.connected ? (status.claude.mode === 'api-key' ? 'explicit API-key mode' : 'signed in' + (status.claude.subscription ? ' · ' + status.claude.subscription : '')) : status.claude.error || 'sign in required') + ' · Codex: ' + (status.codex.connected ? 'connected' : status.codex.error || 'sign in required'); };
        connection.textContent = 'Checking your sign-ins…'; void check().catch(error => { connection.textContent = (error as Error).message; });
        auth.append(action('Check again', check), action('Sign in with Claude', async () => { const result = await api.post('/api/providers/claude/login'); connection.textContent = result.message; }), action('Connect Codex', async () => { const result = await api.post('/api/providers/login'); const link = node('a', 'Continue Codex sign-in'); link.href = result.authUrl; link.target = '_blank'; link.rel = 'noopener noreferrer'; auth.append(link); }));
