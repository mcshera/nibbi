...
import { leadBusy, activeTurns } from './session.js';
...
export const status = (): Record<string, unknown> => ({ ...loadState(), app: 'nibbi', version: '0.8.0', protocolVersion: 1, pid: process.pid, busy: leadBusy(), activeTurn: activeTurns().map(({ runId, provider, steerable }) => ({ runId, provider, steerable }))[0] ?? null, uptimeSec: Math.round((Date.now() - started) / 1000), vault: config.vaultDir });
...
export function snapshot(): unknown { return { cursor: runtime().cursor(), status: status(), fixers: listFixers().map(run => ({ ...run, github: githubBuildSummary(run.id) })), auto: autoView(), goals: goals(), progress: progressSummary() }; }
...
export function runEvents(id: string, attemptId?: string): Array<{ ts: string; kind: string; text: string } & Record<string, unknown>> {
  return (runtime().db.prepare("SELECT type,at,payload FROM events WHERE run_id=? AND (? IS NULL OR json_extract(payload,'$.attemptId')=?) ORDER BY id DESC LIMIT 250").all(id, attemptId ?? null, attemptId ?? null) as { type: string; at: number; payload: string }[]).reverse().map(row => {
    ...
    const tool = row.type === 'tool.started' || row.type === 'tool.finished';
    return { ts: new Date(row.at).toISOString(), kind: row.type, text: String(payload.text ?? payload.summary ?? payload.name ?? (payload.run as { status?: string })?.status ?? ''),
      ...(payload.name !== undefined ? { name: String(payload.name) } : {}), ...(payload.attemptId !== undefined ? { attemptId: String(payload.attemptId) } : {}),
      ...(tool ? { phase: row.type === 'tool.finished' ? 'finished' : 'started', source: payload.source, input: payload.input, ok: payload.ok, summary: payload.summary, error: payload.error, elapsedMs: payload.elapsedMs, bytes: payload.bytes, diff: payload.diff } : {}) };
  });
}
