...
import { archiveThread, createThread, renameThread } from './threads.js';
  ...
  try {
    ...
    try {
      switch (command.name) {
        ...
        case 'session.reset': resetSession(a.threadId === undefined ? undefined : { project: command.projectId, threadId: text(a.threadId) }); message = 'Fresh working context; vault memory is preserved'; break;
        case 'thread.create': data = createThread(command.projectId, a.title === undefined ? undefined : text(a.title)); break;
        case 'thread.rename': data = renameThread(command.projectId, text(a.id), text(a.title)); break;
        case 'thread.archive': data = archiveThread(command.projectId, text(a.id), a.archived !== false); break;
