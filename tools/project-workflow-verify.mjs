...
try{
 ...
 for(const [w,h]of[[1440,900],[390,844],[320,568],[390,430]]){
  ...
  try{await check(`responsive sections and composer ${w}x${h}`,async()=>{
   ...
   await openDock();const mic=page.locator('#mic'),box=await mic.boundingBox();assert(box.width>=44&&box.height>=44);await mic.click();await page.waitForFunction(()=>nibbiApp.voice.snapshot().phase==='paused');assert.equal(await page.locator('#dock-menu').isVisible(),false,'choosing a row closes the panel');assert.equal(await page.locator('.mode[data-mode="mic"]').count(),1,'Hey Nibbi shows as a chip while on');await openDock();await mic.click();await page.waitForFunction(()=>nibbiApp.voice.snapshot().phase==='off');assert.equal(await page.locator('.mode[data-mode="mic"]').count(),0,'chip leaves with the mode');assert.equal(await page.evaluate(()=>workflowMicrophone.requests),1);assert.equal(await page.evaluate(()=>workflowMicrophone.tracks.every(t=>t.readyState==='ended')),true);assert.equal(fixture.calls.some(r=>r.path==='/api/transcribe'),false);
