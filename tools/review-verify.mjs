...
try {
  ...
  await withPage(async (page, _control, calls) => {
    ...
    assert.equal(await page.locator('.review .armed').count(), 0); assert.equal(calls.length, 0);
    await page.keyboard.press('Escape'); await page.locator('#status').focus();
    for (const key of ['a', 'x', 'j', 'ArrowRight']) await page.keyboard.press(key);
    assert.equal(await page.evaluate(() => window.nibbiApp.state().review.i), 0, 'Focused rail buttons own review shortcuts');
    assert.equal(await page.locator('.review .armed').count(), 0); assert.equal(calls.length, 0);
    assert.equal(await page.evaluate(() => document.activeElement.id), 'status');
    await page.keyboard.press('Space'); await page.locator('.margin-card:not([hidden]) #st-motion').waitFor();
    assert.equal(await page.evaluate(() => window.nibbiApp.state().review.i), 0, 'Space opens Settings without changing review');
  });
