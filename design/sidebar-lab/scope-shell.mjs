/** The Scope shell — everything four variants share.
 *
 *  Scope was the owner's pick: "I like 02 scope, can you make chat/builds/plan/issues one row."
 *  The strip is the only thing that differs between the variants, so it is the only thing they write.
 *  Everything below — the switcher, the full-width dropdown, the body, the foot, the cards, the
 *  host-scoped listeners — is written once here.
 *
 *  A variant supplies { id, name, tagline, answers, risks, strip, ... } and gets back { meta, mount }.
 *  strip(ctx) returns the row's buttons; ctx carries the project, the current view, the model, the
 *  improvement flags, onAction, and helpers for the two things every strip needs: a tab's short count
 *  and its full sentence.
 */
import {
...
const PINNED = ['workspace-sidebar', 'sidebar-toggle', 'sidebar-collapse', 'sidebar-backdrop', 'project-rail',
  'settings-rail', 'sidebar-progress', 'status', 'margin-new', 'project-id', 'project-options', 'project-section',
  'section-current', 'section-badge', 'project-thread-new', 'workspace-left'];
...
export function createScopeBar(variant) {
  const meta = {
    id: variant.id, name: variant.name, tagline: variant.tagline,
    pattern: variant.pattern || 'scoped header + full-width dropdown (t3code Sidebar V2)',
    references: variant.references || REFERENCES,
    answers: variant.answers, risks: variant.risks,
    flags: variant.flags || ['chatTab', 'pinnedNew', 'summaryLine', 'progressFoot', 'hoverGear', 'rollupText', 'threadActions'],
    pinned: variant.pinned || PINNED,
  };
  function mount(host, { model, state = 'home', viewport, reduced = false, flags = {}, onAction } = {}) {
  loadCss(new URL('./scope-shell.css', import.meta.url).href);
  loadCss(new URL(`./options/${variant.id}.css`, import.meta.url).href);
  let M = model || { projects: [] };
  let V = viewport || { width: 1180, height: 760, narrow: false };
  let F = { ...flags };
  let open = true, menuOpen = false, query = '', dead = false;

  const root = node('div', `scope-root sidebar-lab-${variant.id}`);
  ...
  /** The one thing a variant owns. It gets everything it needs and returns the row's buttons. */
  function renderTabs(project) {
    const view = project && M.view?.project === project.id ? M.view.section : null;
    const cells = variant.strip({
      project, view, model: M, flags: F, narrow: !!V.narrow, onAction,
      sections: SECTIONS,
      // "71 need attention" → "71"; "No issues" → ""; "56/58 tasks" → "56/58". The sentence stays
      // in the title, the accessible name and the body — the strip only ever carries the number.
      count: section => {
        const badge = project?.sections?.[section]?.badge || '';
        const match = badge.match(/^(\d+(?:\/\d+)?)/);
        return match ? match[1] : '';
      },
      phrase: section => text(project?.sections?.[section]?.badge, ''),
      tone: section => project?.sections?.[section]?.tone || 'quiet',
      accessible: section => text(project?.sections?.[section]?.accessible, ''),
      openChat: () => {
        const threads = project?.threads || [];
        const thread = threads.find(t => t.active) || threads[0];
        if (project && thread) onAction?.('thread', project.id, thread.id);
      },
      openSection: section => project && onAction?.('projectSection', project.id, section),
