// Round-4 shell: mounts five new live technique modules (cellular automaton, agent sim, rigid tiles, strand physics, flip-dot board) and drives them with one clock.
import { ACTIONS, ACTION_IDS } from '../actions.mjs';
const TECH_IDS = ['sand', 'slime', 'shards', 'puff', 'flipdot'];
const TINT = '#7a4b2a';
...
async function load() {
  ...
  mods.forEach(({ id, m, error }, i) => {
    ...
    const fl = node.querySelector('.fun ul'); for (const f of m.meta.fun || []) { const li = document.createElement('li'); li.textContent = f; fl.appendChild(li); }
    const short = node.querySelector('.shortlist input'); short.checked = localStorage.getItem(`nibbi-character-lab-r4:${id}`) === '1'; short.addEventListener('change', () => localStorage.setItem(`nibbi-character-lab-r4:${id}`, short.checked ? '1' : '0'));
