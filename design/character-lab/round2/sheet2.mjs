// node design/character-lab/round2/sheet2.mjs <id> [...] [--reduced] [--tint=#7a4b2a] [--matrix] → round2/evidence/sheet-<id>.png
import { chromium } from 'playwright';
import { fileURLToPath } from 'node:url';
import { mkdir } from 'node:fs/promises';
import { listen } from '../serve.mjs';
const args = process.argv.slice(2); const ids = args.filter(a => !a.startsWith('--'));
if (!ids.length) { console.error('usage: node sheet2.mjs <technique-id> [...]'); process.exit(2); }
...
try {
  ...
  await page.goto(`${url}round2/sheet2.html?${qs}`);
  ...
  console.log(`wrote design/character-lab/round2/evidence/${name}`);
