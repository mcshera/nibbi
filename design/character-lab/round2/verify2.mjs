// Browser checks + screenshots for the round-2 shell. node design/character-lab/round2/verify2.mjs
...
try {
  ...
  await page.goto(url + 'round2/'); await page.waitForFunction(() => window.techLab?.ready, null, { timeout: 60000 }); await page.evaluate(() => document.fonts.ready);
  ...
  await page.screenshot({ path: out + 'lab2-desktop.png', fullPage: true });
  for (const a of ['think', 'work', 'success', 'error', 'sleep']) { await page.evaluate(a => techLab.cue(a), a); await page.waitForTimeout(a === 'success' ? 700 : 1300); await page.screenshot({ path: out + `lab2-${a}.png`, clip: { x: 0, y: 900, width: 1600, height: 620 } }); }
  ...
  await page.screenshot({ path: out + 'lab2-reduced.png', clip: { x: 0, y: 900, width: 1600, height: 620 } });
  ...
  await mobile.goto(url + 'round2/'); await mobile.waitForFunction(() => window.techLab?.ready, null, { timeout: 60000 }); await mobile.waitForTimeout(800);
  ...
  await mobile.screenshot({ path: out + 'lab2-mobile.png', fullPage: true }); check('Mobile 390 px: no horizontal overflow');
