...
const TECH_IDS = ['bead', 'marble', 'inkwell', 'nibdrop', 'velvet', 'colony'];
const TINT = '#7a4b2a';
...
const cards = [];
const COPY = ['merged 8.2 — the diff viewer now blocks consecutive adds and deletes', 'listening. say the bug the way you would tell a friend', 'thinking about the plan — three milestones left, two look parallel', 'fixer 2 is on the tests; I will post when it lands', 'that one failed at 80 turns. the worktree is kept, nothing merged', 'good morning. two runs landed overnight; one wants your eyes'];
...
async function load() {
  ...
  mods.forEach(({ id, m, error }, i) => {
    ...
    const short = node.querySelector('.shortlist input'); short.checked = localStorage.getItem(`nibbi-character-lab-r3:${id}`) === '1'; short.addEventListener('change', () => localStorage.setItem(`nibbi-character-lab-r3:${id}`, short.checked ? '1' : '0'));
