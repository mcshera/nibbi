// Round-5 shell: mounts five thoughtful versions (gaze model, rocking keel, diffusion memory, contour focus, turning pebble) and drives them with one clock.
// Adds a composer that plays the beat live: focus → listen, Enter → think → the reply (hello).
import { ACTIONS, ACTION_IDS } from '../actions.mjs';
const TECH_IDS = ['regard', 'keel', 'tide', 'contour', 'pebble'];
const TINT = '#7a4b2a';
const HERO_R = 84, PILL_R = 26, TINY_R = 12;
const $ = s => document.querySelector(s);
const state = { paused: false, speed: 1, energy: 1, tinted: false, reduced: matchMedia('(prefers-reduced-motion: reduce)').matches, tour: null, action: 'idle', beat: null };
const cards = [];
const COPY = [
  'you said the tests were flaky, not failing — that changes what I would fix first',
  'listening. take your time',
  'give me a second with that — two of the three milestones overlap',
  'fixer 2 is on it. I will say when it lands, not before',
  'that one failed at 80 turns. my call, and the wrong one — the worktree is kept',
];
...
async function load() {
  ...
  mods.forEach(({ id, m, error }, i) => {
    ...
    node.querySelector('.faculty-text').textContent = m.meta.faculty; node.querySelector('.technique-text').textContent = m.meta.technique;
    ...
    const tl = node.querySelector('.thoughtful ul'); for (const f of m.meta.thoughtful || []) { const li = document.createElement('li'); li.textContent = f; tl.appendChild(li); }
    const short = node.querySelector('.shortlist input'); short.checked = localStorage.getItem(`nibbi-character-lab-r5:${id}`) === '1'; short.addEventListener('change', () => localStorage.setItem(`nibbi-character-lab-r5:${id}`, short.checked ? '1' : '0'));
    ...
    for (const [who, copy, tint] of [[`NIBBI <span class="badge">${m.meta.name.toUpperCase()}</span>`, COPY[i % COPY.length], null], [`<span style="color:${TINT}">FIXER 2</span> <span class="badge">${m.meta.name.toUpperCase()} · TINTED</span>`, 'staged a-fix-diff-viewer · 3 files · verify passed', TINT]]) {
      const row = document.createElement('div'); row.className = tint ? 'row fixer-row' : 'row'; if (tint) row.style.display = state.tinted ? '' : 'none'; const host = document.createElement('div'); host.className = 'strip-host'; const body = document.createElement('div'); body.style.minWidth = '0';
...
function cue(action, { fromTour = false, fromBeat = false } = {}) {
  if (!fromTour && state.tour) stopTour();
  if (!fromBeat && state.beat) stopBeat();
...
const TOUR = [['hello', 2.2], ['listen', 5], ['think', 5], ['work', 5], ['success', 2.6], ['error', 3.2], ['sleep', 4], ['idle', 0]];
...
// the beat: the composer plays listen → think → reply on every version at once
function stopBeat() { if (state.beat?.timer) clearTimeout(state.beat.timer); state.beat = null; $('#beat-note').textContent = 'focus → listen · enter → think → reply'; }
function beatReply(text) {
  stopBeat(); state.beat = { timer: null };
  cue('think', { fromBeat: true }); $('#beat-note').textContent = 'thinking about it…';
  const hold = Math.min(3.2, 1.4 + text.length * .03) * 1000 / state.speed;   // longer questions take a longer beat
  state.beat.timer = setTimeout(() => { if (!state.beat) return; cue('hello', { fromBeat: true }); $('#beat-note').textContent = 'the reply lands'; state.beat.timer = setTimeout(() => { if (!state.beat) return; state.beat = null; cue($('#say') === document.activeElement ? 'listen' : 'idle', { fromBeat: true }); $('#beat-note').textContent = 'focus → listen · enter → think → reply'; }, 1800 / state.speed); }, hold);
}
...
$('#tinted').addEventListener('change', e => { state.tinted = e.target.checked; for (const c of cards) c.node.querySelector('.size.tinted').style.display = state.tinted ? '' : 'none'; for (const r of document.querySelectorAll('#chat-rows .fixer-row')) r.style.display = state.tinted ? '' : 'none'; });
const say = $('#say');
say.addEventListener('focus', () => { if (!state.beat) cue('listen', { fromBeat: true }); });
say.addEventListener('blur', () => { if (!state.beat && state.action === 'listen') cue('idle', { fromBeat: true }); });
say.addEventListener('keydown', e => { if (e.key === 'Enter') { e.preventDefault(); const text = say.value.trim(); say.value = ''; beatReply(text); } else if (e.key === 'Escape') { say.blur(); } });
document.addEventListener('keydown', e => { if (e.target.matches('input,select,textarea')) return; const n = Number(e.key); if (n >= 1 && n <= ACTIONS.length) { cue(ACTIONS[n - 1].id); e.preventDefault(); return; } if (e.key === ' ') { $('#pause').click(); e.preventDefault(); } if (e.key === 't') playTour(); if (e.key === '/') { say.focus(); e.preventDefault(); } });
document.addEventListener('visibilitychange', () => { last = performance.now(); });
requestAnimationFrame(frame);

window.techLab = {
  get ready() { return cards.length === TECH_IDS.length; }, get action() { return state.action; }, techniques: TECH_IDS, actions: ACTIONS.map(a => ({ id: a.id, label: a.label })),
