// node design/character-lab/round5/tech.test.mjs <id> [id...] — headless-browser contract checks for round-5 versions (attention, calm, the beat, stills, reduced motion, touch, tint, perf).
...
try {
  for (const id of ids) {
    ...
    await page.goto(`${url}round5/tech-test.html?id=${id}`);
    ...
    if (res.measures) console.log(`     ${id}: measures ${JSON.stringify(res.measures)}`);
    failed += res.failures.length + errors.length + res.errors.length;
    await page.close();
  }
} finally { await browser.close(); server.close(); }
console.log(`\n${failed ? failed + ' failure(s)' : 'all checks passed'} across ${ids.length} version(s)`);
