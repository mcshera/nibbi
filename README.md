...
| `/artifacts [project]` · `/report [hours]` · `/history <q>` · `/vault <path>` · `/model [name]` · `/help` | proof of work, overnight report, search, brain files, model |
...
approve & merge` (or `log · requeue`). Agent cards on the pill carry the same actions; foremen get `pause/resume` and the
